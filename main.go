...
import (
	...
	_ "github.com/piriya-muaithaisong/Simple-Bank/doc/statik"
...
func runTaskProcessor(redisOpt asynq.RedisClientOpt, store db.Store) {
	...
	if err != nil {
		log.Fatal().Err(err).Msg("faled to start task processor")
	}
...
func runGatewayServer(config utils.Config, store db.Store, taskDistributor worker.TaskDistributor) {
	...
	if err != nil {
		log.Fatal().Err(err).Msg("cannot register error handler: ")
	}
	...
	statikFS, err := fs.New()
	if err != nil {
		log.Fatal().Err(err).Msg("cannot get statik fs: ")
	}
